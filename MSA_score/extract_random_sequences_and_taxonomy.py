...
def pick_random_sequences(input_fasta, num_sequences):
    ...
    sequence_ids = [seq.id for seq in random_sequences]
    
    return random_sequences, sequence_ids

def write_sequences_to_fasta(sequences, output_fasta):
    with open(output_fasta, "w") as output_handle:
        SeqIO.write(sequences, output_handle, "fasta")

def extract_taxonomy_data(taxonomy_file, sequence_ids, output_taxonomy_file):
    taxonomy_data = {}
...
def main(input_fasta, taxonomy_file, output_fasta, output_taxonomy_file, num_sequences):
    random_sequences, sequence_ids = pick_random_sequences(input_fasta, num_sequences)
    
    write_sequences_to_fasta(random_sequences, output_fasta)
    extract_taxonomy_data(taxonomy_file, sequence_ids, output_taxonomy_file)
